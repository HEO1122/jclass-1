		...
		*/
		...
		/*
			1. Pattern 클래스를 이용해서 검사하고 싶은 정규식 문법을 만든다.
				참고 ]
					이 클래스는 생성자 함수의 접근지정자가 private로 감춰놓은 경우에 해당한다.
					그리고 이 클래스는 속성이 final 속성이 적용되어있고
					의미는 이 클래스를 더이상 상속받은 클래스는 만들 수 없다 라는 의미이다.
					
					따라서 이 클래스는 외부에서  new 시켜서 사용할 수 없는 클래스이고
					상속 시켜서 그 클래스의 인스턴스를 변수로 받아서 사용할 수도 없다.
					이경우 이 클래스의 객체를 사용하려면
					소속 함수중 속성이 static 이면서 반환값이 Pattern 인 함수를 사용하면 된다.
		...
		if(bool) {
			System.out.println("*** 올바른 휴대폰 번호 입니다! ***");
		} else {
